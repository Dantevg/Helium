...
function Node:Style(style, node)
	node = node or self
	return node == self and self.style[style] or (self.root and self.root:Style(style, node))
end
...
function Node:sibling(n)
	...
	return self.id and self.parent.nodes[self.id+(n or 0)]
end
